	...
	public partial class PayRoll : BaseForm
	{
		...
		private void CalculatePay()
		{
			try
			{
				...
				if (!decimal.TryParse(BaseSalaryTb.Text, out decimal baseSalary)) return;
				if (!int.TryParse(DWTb.Text, out int daysWorked)) return;
				if (!decimal.TryParse(OverHrsTb.Text, out decimal overtimeHours)) return;
				if (!decimal.TryParse(AllowanceTb.Text, out decimal allowance)) return;
		...
		private string SafeDisplay(string value, string defaultValue = "0.00")
		{
		...
		private void WorkScheduleCb_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (decimal.TryParse(BaseSalaryTb.Text, out decimal currentBase))
			{
